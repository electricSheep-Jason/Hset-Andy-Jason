...
//check function if you can rotate, move left/right e.g. at edge
...
public class Tetris extends Application implements Initializable {
    ...
    public int score;
    public boolean game = false; //if game = false, nothing happens, if game = true, game starts automatically
    public int level = 1;
    ...
    public void MoveDown(Form form) {
        ...
        //if shape reaches the bottom or hits another shape
        //its position is saved in the list
        //it is fixed there

        // Y_max is height of page
        //size of block
        if (form.a.getY() == Y_max - size || form.b.getY() == Y_max - size //checks if shape is at bottom
                || form.c.getY() == Y_max - size || form.d.getY() == Y_max - size //check if shape is at bottom
    ...
    public void InstantMoveDown (Form form) {
            //logic: if someone presses Enter
            //shape "reappears"/"moves" to the bottom
            //while loop of MoveDown: if someone presses Enter, the move down function repeats until it hits bottom/hit another shape

            //false means block has not hit bottom yet
            while (form.a.getY() != Y_max - size && form.b.getY() != Y_max - size
            ...
            public boolean CheckLegalMoveDown(Form form){
                ...
                //check if it hits something, not if block is at bottom. if it hits another block, returns true.
            ...
            public void Start () {
                ...
                timeline.setCycleCount(Animation.INDEFINITE);
                timeline.play();
                StartButton.setDisable(true); //after pressing start button, it is disabled
                PlayAudio(true);
            }
            ...
            public void Pause () {
                if (timeline != null) {
                    timeline.pause();
                    ButtonBack.setVisible(true);
                    StartButton.setDisable(false); //Start button is enabled after pressing Pause
                }
            }
            ...
            /*
            The following are a chunk of sound effects
             */

            public void PlayAudio (boolean sound) {

                     AudioClip backgroundmusic = new AudioClip(this.getClass().getResource("newbgmusic.wav").toString());
                     backgroundmusic.setCycleCount(45);
                     if (sound == true) {
            ...
            public void MoveSoundEffect() {
                AudioClip moveblock = new AudioClip(this.getClass().getResource("moveblock.wav").toString());
                moveblock.play();

            }

            public void InstantMoveSoundEffect() {
                AudioClip instantmove = new AudioClip(this.getClass().getResource("instantmove.wav").toString());
                instantmove.play();

            }

            public void ClearLineSoundEffect() {
                AudioClip clearline = new AudioClip(this.getClass().getResource("clearline.wav").toString());
                clearline.play();
